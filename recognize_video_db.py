...
while True:
    ...
    frameorg = vs.read()
    count=count+1
    frame=increase_brightness(frameorg)
    ...
    (h, w) = frame.shape[:2]
    frameorg = imutils.resize(frameorg, width=600)
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            if proba*100>100.0:        
                    ...
                    cv2.rectangle(frameorg, (startX, startY), (endX, endY),
                        (0, 255, 0), 2)
                    cv2.putText(frameorg, text+" ID: "+str(i+1), (startX, y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 2)
                    #text1 = cv2.putText(frame, "Number of faces: "+str(i+1), (0, frame.shape[0] - 10), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1)
                    numoffaces=i+1
                    text2 = cv2.putText(frameorg, "Frame count: "+str(count), (frame.shape[0], frame.shape[0] - 400), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1)
                    text3 = cv2.putText(frameorg, "Maximum number of faces counted: "+str(maxval), (0, frame.shape[0]-25), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1)
                    text4 = cv2.putText(frameorg, "Neutral Count: "+str(neutralcount)+" Frown Count: "+str(frowncount)+" Happy Count: "+str(happycount), (0, frame.shape[0]-40), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1)
            else:
                    # draw the bounding box of the face along with the
                    # associated probability
                    text = name_exp
    ...
    fps.update()
    text1=cv2.putText(frameorg, "Number of faces: "+str(numoffaces), (0, frame.shape[0] - 10), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1)
    # show the output frame
    cv2.imshow("frameorg", frameorg)
    #cv2.imshow("frame", text3)
    cv2.imshow("frameorg", text1)
